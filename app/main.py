...
eighth_rest_imgs = [cv2.imread(eighth_rest_file, 0)
                    for eighth_rest_file in eighth_rest_files]
quarter_rest_imgs = [cv2.imread(quarter_rest_file, 0)
                     for quarter_rest_file in quarter_rest_files]
half_rest_imgs = [cv2.imread(half_rest_file, 0)
                  for half_rest_file in half_rest_files]
whole_rest_imgs = [cv2.imread(whole_rest_file, 0)
                   for whole_rest_file in whole_rest_files]
...
if __name__ == "__main__":
    ...
    staff_recs = locate_images(
        img_gray, staff_imgs, staff_lower, staff_upper, staff_thresh)
    ...
    sharp_recs = locate_images(
        img_gray, sharp_imgs, sharp_lower, sharp_upper, sharp_thresh)
    ...
    natural_recs = locate_images(
        img_gray, natural_imgs, natural_lower, natural_upper, natural_thresh)
    ...
    flat_recs = locate_images(
        img_gray, flat_imgs, flat_lower, flat_upper, flat_thresh)
    ...
    eighth_rest_recs = merge_recs(
        [j for i in eighth_rest_recs for j in i], 0.5)
    ...
    quarter_rest_recs = merge_recs(
        [j for i in quarter_rest_recs for j in i], 0.5)
    ...
    quarter_recs = locate_images(
        img_gray, quarter_imgs, quarter_lower, quarter_upper, quarter_thresh)
    ...
    half_recs = locate_images(
        img_gray, half_imgs, half_lower, half_upper, half_thresh)
    ...
    whole_recs = locate_images(
        img_gray, whole_imgs, whole_lower, whole_upper, whole_thresh)
